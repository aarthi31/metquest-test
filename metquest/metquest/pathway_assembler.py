# -*- coding: utf-8 -*-
"""
Created on Sun Apr 29 18:14:24 2018
...
def find_pathways(G, seed_mets_input, path_len_cutoff, *args):
    ...
    timetaken : float
        Time taken in seconds for the pathways to be identified
    """
            ...
            if mets_needed:
                for val in range(currentcolumnidx-1,
                    ...
                    if val <= len(mets_needed)*(currentcolumnidx-2):
                        first_round_calculations(
                            mets_needed, currentcolumnidx, rxns, val)
                    else:
                        second_round_calculations(
                            mets_needed, currentcolumnidx, rxns, val)
    toc = clock()
    timetaken = toc - TIC
    print('Time taken', timetaken)
    return pathway_table, cyclic_pathways, timetaken, scope


def first_round_calculations(mets_needed, currentcolumnidx, rxns, val):
    """
            ...
            if onemetnotfound != 'Y':
                ...
                if set(currentmetcomb).issubset(pathway_table):
                    ...
                    for partitions in all_partitions:
                        find_all_rxn_combination_firstround(rxns, partitions, other_mets_not_in_comb,
                                                            temp_rxn_list, number_of_pathways_found,
                                                            currentcolumnidx)


def find_all_rxn_combination_firstround(rxns, partitions, other_mets_not_in_comb,
    ...
    if counter == len(other_mets_not_in_comb):
        if all([prod(list(number_of_pathways_found.values())) > maxnumpath,
            ...
            populate_table(rxns, temp_rxn_list_current, currentcolumnidx)


def populate_table(rxns, temp_rxn_list_current, currentcolumnidx):
    """
...
def second_round_calculations(mets_needed, currentcolumnidx, rxns, val):
    """
    ...
    for partitions in all_partitions:
        ...
        if counter_new == len(mets_needed):
            if all([prod(list(number_of_pathways_found.values())) > maxnumpath,
                ...
                populate_table(rxns, temp_rxn_list, currentcolumnidx)
